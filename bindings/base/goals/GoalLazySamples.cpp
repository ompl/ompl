...
void ompl::binding::base::initGoals_GoalLazySamples(nb::module_ &m) {
    nb::class_<ob::GoalLazySamples, ob::GoalStates>(m, "GoalLazySamples")
        ...
        .def("isSampling", &ob::GoalLazySamples::isSampling,
             "Return true if the background sampling thread is alive.")
        ...
        .def("setNewStateCallback",
             &ob::GoalLazySamples::setNewStateCallback,
             nb::arg("callback"),
        )

        // lower‐level control:
        .def("addStateIfDifferent", &ob::GoalLazySamples::addStateIfDifferent,
             nb::arg("state"), nb::arg("minDistance"),
        ...
        .def("hasStates", &ob::GoalLazySamples::hasStates,
             "True if any goal states (fixed or sampled) exist.")
        .def("getState", &ob::GoalLazySamples::getState, nb::arg("index"),
             nb::rv_policy::reference_internal,
             "Get the goal state at index (0 ≤ index < getStateCount()).")
        .def("getStateCount", &ob::GoalLazySamples::getStateCount,
             "Number of goal states accumulated so far.")

        // inherited clear
        .def("clear", &ob::GoalLazySamples::clear,
             "Remove all fixed and sampled goal states.");
