...
ompl::base::NumericalCollisionTrajOptConstraint::NumericalCollisionTrajOptConstraint(
    ompl::base::StateValidityCheckerPtr sv, ompl::base::StateSpacePtr ss, const sco::VarVector &vars, double safeDist)
  : ConstraintFromFunc(sco::VectorOfVectorPtr(new ObstacleDistanceFunction(sv, ss, safeDist)), vars,
                       Eigen::VectorXd::Ones(vars.size() / ss->getDimension()) * 1.0,  // ss->getDimension(),
                       sco::ConstraintType::INEQ, "ObsConstraint")
{
}

/************ Jacobian Collision Constraint Implementation. **************/

ompl::base::JacobianCollisionTrajOptConstraint::JacobianCollisionTrajOptConstraint(double safeDist,
                                                                                   WorkspaceCollisionFn collision,
                                                                                   StateSpacePtr ss, JacobianFn J,
                                                                                   sco::VarVector vars)
  : eval_(new JacobianDiscreteCollisionEvaluator(collision, ss, J, vars)), safeDist_(safeDist)
{
    name_ = std::string("JacoColl ") + vars[0].var_rep->name;
}

sco::ConvexConstraintsPtr ompl::base::JacobianCollisionTrajOptConstraint::convex(const std::vector<double> &x,
                                                                                 sco::Model *model)
{
...
ompl::base::JacobianContinuousTrajOptConstraint::JacobianContinuousTrajOptConstraint(
    double safeDist, WorkspaceContinuousCollisionFn collision, StateSpacePtr ss, JacobianFn J, sco::VarVector vars0,
    sco::VarVector vars1)
  : eval_(new JacobianContinuousCollisionEvaluator(collision, ss, J, vars0, vars1)), safeDist_(safeDist)
{
    name_ = "Jaco ContColl";
}

sco::ConvexConstraintsPtr ompl::base::JacobianContinuousTrajOptConstraint::convex(const std::vector<double> &x,
                                                                                  sco::Model *model)
{
    sco::ConvexConstraintsPtr out(new sco::ConvexConstraints(model));
    std::vector<sco::AffExpr> exprs = eval_->calcDistanceExpressions(x);
...
ompl::base::ObstacleConstraint::ObstacleConstraint(const ompl::base::SpaceInformationPtr &si, double safeDist,
  ...
  : ConvexifiableConstraint(si)
  , continuousCollision_(collision)
  , J_(J)
  , useJacobians_(true)
  , continuous_(true)
  , safeDist_(safeDist)
...
ompl::base::Cost ompl::base::ObstacleConstraint::stateCost(const State *s) const
{
    ...
    if (useJacobians_)
    {
        sco::ConstraintPtr constraint(
            new JacobianCollisionTrajOptConstraint(safeDist_, collision_, si_->getStateSpace(), J_, fake_variables));
        return Cost(sco::vecSum(constraint->value(state)));
    }
    else
    {
        sco::ConstraintPtr constraint(new NumericalCollisionTrajOptConstraint(
            si_->getStateValidityChecker(), si_->getStateSpace(), fake_variables, safeDist_));
        return Cost(sco::vecSum(constraint->value(state)));
    }
}
...
ompl::base::Cost ompl::base::ObstacleConstraint::motionCost(const State *s1, const State *s2) const
{
    ...
    if (continuous_)
    {
        ...
        sco::ConstraintPtr constraint(new JacobianContinuousTrajOptConstraint(
            safeDist_, continuousCollision_, si_->getStateSpace(), J_, fake_variables_0, fake_variables_1));
...
std::vector<sco::ConstraintPtr> ompl::base::ObstacleConstraint::toConstraint(sco::OptProbPtr problem)
{
    ...
    for (int i = 0; i < nSteps; i++)
    {
        ...
        {
            sco::ConstraintPtr constraint(new NumericalCollisionTrajOptConstraint(
                si_->getStateValidityChecker(), si_->getStateSpace(),
                std::static_pointer_cast<ompl::geometric::OmplOptProb>(problem)->GetVarRow(i), safeDist_));
            constraints.push_back(constraint);
        }
    }
    return constraints;
