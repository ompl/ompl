...
LocalMinimaTree::LocalMinimaTree(std::vector<base::SpaceInformationPtr> siVec):
  siVec_(siVec)
{
...
void LocalMinimaTree::setSelectedMinimumNext()
{
  ...
  if(selectedMinimum_.size() > 0)
  {
      ...
      if(maxMinima > 0)
      {
          if(selectedMinimum_.back() < maxMinima-1) selectedMinimum_.back()++;
          else selectedMinimum_.back() = 0;
          OMPL_DEVMSG1("Selected local minimum %d/%d (level %d)", 
              selectedMinimum_.back()+1, maxMinima, selectedMinimum_.size()-1);
      }
  }
...
const ompl::base::PathPtr&
LocalMinimaNode::asPathPtr() const
{
  if(hasPathPtrRepresentation) return path_;
...
const StatesPath& 
LocalMinimaNode::asStates() const
{
  if(hasStatesRepresentation) return spath_;

  OMPL_ERROR("NYI");
  throw ompl::Exception("NYI");
}
const VertexPath& 
LocalMinimaNode::asVertices() const
{
  if(hasVertexRepresentation) return vpath_;
  OMPL_ERROR("NYI");
  throw ompl::Exception("NYI");
}
