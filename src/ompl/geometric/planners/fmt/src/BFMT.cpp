...
namespace ompl
{
    namespace geometric
    {
        ...
        void BFMT::setup(void)
        {
            if (pdef_)
            {
                ...
                nn_->setDistanceFunction(
                    std::bind(&BFMT::distanceFunction, this, std::placeholders::_1, std::placeholders::_2));
        ...
        void BFMT::saveNeighborhood(std::shared_ptr<NearestNeighbors<BiDirMotion *>> nn, BiDirMotion *m)
        {
                    ...
                    if (nbh[j]->getCurrentSet() == BiDirMotion::SET_CLOSED)
                    {
                        if (nearestK_)
                        {
                            ...
                            const base::Cost worstCost =
                                opt_->motionCost(neighborhoods_[nbh[j]].back()->getState(), nbh[j]->getState());
            ...
            for (std::size_t i = 0; i < nbh.size(); ++i)
            {
                ...
                else
                {
                    ...
                    if (it != neighborhoods_.end())
                    {
                        ...
                        const base::Cost worstCost =
                            opt_->motionCost(it->second.back()->getState(), nbh[i]->getState());
                        ...
                        else
                        {
                            ...
                            for (std::size_t j = 0; j < nbhToUpdate.size(); ++j)
                            {
                                ...
                                const base::Cost cost =
                                    opt_->motionCost(nbh[i]->getState(), nbhToUpdate[j]->getState());
