...
bool PathSpaceSparseOptimization::getSolution(ob::PathPtr &solution)
{
    if (hasSolution_)
    {
        if (!isDynamic())
...
void PathSpaceSparseOptimization::getPlannerData(ob::PlannerData &data) const
{
    if (isDynamic())
    {
        if (!data.hasControls())
...
void PathSpaceSparseOptimization::freePath(std::vector<ob::State *> path, const ob::SpaceInformationPtr &si) const
{
    for (uint k = 0; k < path.size(); k++)
    {
...
std::vector<ob::State *> PathSpaceSparseOptimization::getProjectedPath(const std::vector<ob::State *> pathBundle,
...
bool PathSpaceSparseOptimization::isProjectable(const std::vector<ob::State *> &pathBundle) const
{
    return (getProjectionIndex(pathBundle) >= 0);
}

int PathSpaceSparseOptimization::getProjectionIndex(const std::vector<ob::State *> &pathBundle) const
{
    if (!hasParent())
    {
        return 0;
...
void PathSpaceSparseOptimization::getPathIndices(const std::vector<ob::State *> &states, std::vector<int> &idxPath) const
{
    if (!hasParent())
    {
        return;
