...
#define ENSURE_SUCCESS(expr)                                                                                           \
    ...
    Cnt GurobiModel::addEqCnt(const AffExpr &expr, const string &name)
    {
        ...
        simplify2(inds, vals);
        ENSURE_SUCCESS(GRBaddconstr(m_model, inds.size(), const_cast<int *>(inds.data()),
                                    const_cast<double *>(vals.data()), GRB_EQUAL, -expr.constant,
                                    const_cast<char *>(name.c_str())));
    ...
    Cnt GurobiModel::addIneqCnt(const QuadExpr &qexpr, const string &name)
    {
        ...
        ENSURE_SUCCESS(GRBaddqconstr(m_model, numlnz, linds.data(), lvals.data(), qexpr.size(), inds1.data(),
                                     inds2.data(), const_cast<double *>(qexpr.coeffs.data()), GRB_LESS_EQUAL,
                                     -qexpr.affexpr.constant, const_cast<char *>(name.c_str())));
        return Cnt();
    }
