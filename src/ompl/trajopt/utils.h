...
namespace sco
{
    ...
    */
    ...
    void TRAJOPT_API AddVarArrays(sco::OptProb &prob, int rows, const vector<int> &cols,
                                  const vector<std::string> &name_prefix, const vector<VarArray *> &newvars);

    void TRAJOPT_API AddVarArray(sco::OptProb &prob, int rows, int cols, const std::string &name_prefix,
                                 VarArray &newvars);
}
