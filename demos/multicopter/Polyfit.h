...
Eigen::VectorXd polyfit_Eigen(std::vector<double> x, const Eigen::VectorXd &y, int degree)
{
    ...
    Eigen::MatrixXd A = Eigen::MatrixXd::NullaryExpr(x.size(), degree + 1, [&x](Eigen::Index i, Eigen::Index j)
...
std::vector<T> polyDerivatives(std::vector<T> &coefficients)
{
    ...
    {
        derivativeCoefficients.push_back((degree)*coefficients.at(degree));  // for ax^(n), derivative coefficient is
                                                                             // a*n
    }
...
/**
 * Taking in an input of polyomial coefficients, from leading power to constant (n->0), and x values that are desired to
 * be evaluated
 *
