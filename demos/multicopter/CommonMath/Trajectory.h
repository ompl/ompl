...
namespace CommonMath
{
    ...
    public:
        ...
        Trajectory(Vec3 c0, Vec3 c1, Vec3 c2, Vec3 c3, Vec3 c4, Vec3 c5, double startTime, double endTime)
          : _coeffs{c0, c1, c2, c3, c4, c5}, _startTime(startTime), _endTime(endTime)
        {
            assert(startTime <= endTime);
        }
        ...
        Vec3 GetVelocity(double t) const
        {
            assert(t >= _startTime);
            assert(t <= _endTime);
            return _coeffs[0] * t * t * t * t * t + _coeffs[1] * t * t * t * t + _coeffs[2] * t * t * t +
                   _coeffs[3] * t * t + _coeffs[4] * t + _coeffs[5];
        }
        ...
        Trajectory operator-(const Trajectory rhs)
        {
            ...
            return Trajectory(_coeffs[0] - rhs[0], _coeffs[1] - rhs[1], _coeffs[2] - rhs[2], _coeffs[3] - rhs[3],
                              _coeffs[4] - rhs[4], _coeffs[5] - rhs[5], startTime, endTime);
        }
