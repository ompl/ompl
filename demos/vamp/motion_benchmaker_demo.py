...
def main(
    ...
    **kwargs,
    ...
    if benchmark:
        ...
        print(
            f"Benchmarking {robot} on first problem of each type with {n_trials} trials..."
        )
        ...
        for i, data in enumerate(pset):
            ...
            for trial in range(trials):
                ...
                if benchmark:
                    ...
                    ompl_benchmark.addExperimentParameter(
                ...
                if visualize_enabled and vis is not None:
                    ...
                    if pointcloud:
                        ...
                        z_normalized = np.clip(
                            (z_values - z_min) / (z_max - z_min), 0, 1
                        )

                        # Smooth rainbow colormap: red -> yellow -> green -> cyan -> blue
                        colors = np.zeros((len(original_pc), 3))
                        colors[:, 0] = np.maximum(
                            0, 1 - 2 * np.abs(z_normalized - 0.25)
                        )  # Red
                        colors[:, 1] = np.maximum(
                            0, 1 - 2 * np.abs(z_normalized - 0.5)
                        )  # Green
                        colors[:, 2] = np.maximum(
                            0, 1 - 2 * np.abs(z_normalized - 0.75)
                        )  # Blue
                        vis.add_point_cloud(original_pc, color=colors, point_size=0.01)
                    else:
                        vis.load_mbm_environment(
                            data, padding=0.0, color=(0.8, 0.4, 0.2, 0.75)
                        )

                    # Convert path to numpy array
                    simplified_path.interpolate(
                        int(simplified_path.getStateCount() * 15)
                    )
