...
The core OMPL library does not include code for state validity checking. This is intentional, since defining this notion depends on the type of problems to be solved. Through the integration with [VAMP](https://github.com/KavrakiLab/vamp) we show _one_ way of performing fast collision checking between manipulators and their environment. The ROS interface to OMPL in [MoveIt](https://moveit.ai) defines state validity checking to be collision checking between the robot model and sensed objects in the environment. If planning for protein folding, the state validity check has to do with the evaluation of energy functions.
