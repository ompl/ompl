...
Check out the [tutorial](constrainedPlanningTutorial.html), and the following demos to learn more. Each of these demos comes with a way to visualize the results of planning. The sphere, torus, and implicit kinematic chain demos each come with a [Blender](https://www.blender.org/) file (`.blend`') that visualize the results of planning in 3-D. These are all in `demos/constraint/visualization`.
:
