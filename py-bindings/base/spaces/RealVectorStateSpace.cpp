...
void ompl::binding::base::initSpaces_RealVectorStateSpace(nb::module_ &m)
{
    ...
    stateType
        ...
        .def(
            "__getitem__", [](const ob::RealVectorStateSpace::StateType *s, unsigned int i) { return s->values[i]; },
            nb::rv_policy::reference_internal)
        .def("__setitem__", [](ob::RealVectorStateSpace::StateType *s, unsigned int i, double v) { s->values[i] = v; })
        ...
        .def("__getitem__",
             [](const ob::RealVectorStateSpace::StateType *s, const nb::slice &slice)
             {
                 ...
                 {
                     throw nb::type_error("Open-ended slices are not supported (unknown state dim)");
                 }
                 ...
                 if (start < 0 || stop < 0)
                 {
                     throw nb::type_error("Negative slice indices require a known dim count");
                 }
                 ...
                 size_t slicelength = static_cast<size_t>((stop - start + step - 1) / step);
        ...
        .def(
            ...
            [](ob::RealVectorStateSpace::StateType *s, const nb::slice &slice, const std::vector<double> &values)
            {
                ...
                {
                    throw nb::value_error("Size of assignment does not match slice size");
                }
    ...
    nb::class_<ob::RealVectorStateSpace, ob::StateSpace>(m, "RealVectorStateSpace")
        ...
        .def("setBounds", nb::overload_cast<const ob::RealVectorBounds &>(&ob::RealVectorStateSpace::setBounds))
        ...
        .def("printState",
             [](const ob::RealVectorStateSpace &space, const ob::State *state) { space.printState(state, std::cout); })
        .def("printSettings", [](const ob::RealVectorStateSpace &space) { space.printSettings(std::cout); })
