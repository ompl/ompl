...
def test_control_planners():
    ...
    for planner_type in (oc.RRT, oc.SST, oc.SSTstar):
        planner = planner_type(si)
        if planner_type in (oc.SST, oc.SSTstar):
            planner.setSelectionRadius(0.2)
            planner.setPruningRadius(0.1)
        if planner_type is oc.SSTstar:
            planner.setInitialIteration(100)
            planner.setShrinkFactor(0.99)
